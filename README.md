...
### Собеседование:
- STT (распознавание речи) с потоковым получением
- TTS (озвучка вопросов) с потоковым синтезом
- **T-pro-it-1.0** - русскоязычная LLM модель от T-Tech для проведения интервью
...
### 🆕 Поддержка модели T-pro-it-1.0:
- **32.8B параметров** на базе Qwen 2.5
- **Отличные результаты** на русских бенчмарках (MERA: 0.629, MaMuRaMu: 0.841)
- **Потоковое получение** ответов для лучшего UX
- **Специализация** на русском языке и технических интервью
- **Интеграция** с VideoSDK для проведения интервью в реальном времени
